...
export default function ClubDashboardClient({ club }: { club: any }) {
  ...
  async function handleDeleteEvent(id: string) {
    ...
    const result = await deleteCommunityEvent(id, club.id);
            ...
            {activeTab === "events" && (
              <div className="space-y-6">
                <div className="flex items-center justify-between gap-4 flex-wrap">
                  <h3 className="font-heading text-lg font-bold tracking-tight flex items-center gap-3">
                    Etkinlikler <span className="bg-primary-fixed text-primary text-[11px] px-2.5 py-0.5 rounded-full font-semibold">{club.events?.length || 0}</span>
                  </h3>
          ...
          {sheetMode === "create_survey" ? (
            ...
            <form key={editingEvent?.id || "new-event"} action={handleEventSubmit} className="p-8 space-y-6">
              <div className="space-y-1.5">
                ...
                <input id="ev-title" name="title" required defaultValue={editingEvent?.title || ""} placeholder="Örn: Yapay Zeka Zirvesi 2026" className={inputClass + " h-12 font-medium"} />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <input id="ev-location" name="location" defaultValue={editingEvent?.location || ""} placeholder="Örn: Ana Oditoryum" className={inputClass} />
              </div>
