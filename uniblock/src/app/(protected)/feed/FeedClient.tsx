            ...
            <aside className="w-full lg:w-[350px] shrink-0 flex flex-col gap-8">
              ...
              <div className="bg-white border-2 border-black p-6">
                <div className="flex justify-between items-center mb-4 border-b-2 border-gray-100 pb-2">
                  ...
                  <span className="text-[10px] font-bold bg-accent text-white px-2 py-1">{initialSurveys.length} AKTİF</span>
                </div>
                ...
                <div className="relative group/scroll">
                  <div className="max-h-[460px] overflow-y-auto pr-1 space-y-6 scrollbar-hide">
                    {initialSurveys.length > 0 ? initialSurveys.map((survey) => (
                      <div key={survey.id} className="border border-gray-200 p-4 relative group">
                        ...
                        <div className="flex justify-between items-start mb-2">
                          <span className="text-[10px] font-bold uppercase tracking-widest text-accent">{survey.club?.name}</span>
                          <span className="text-[9px] text-gray-400 font-bold">{new Date(survey.createdAt).toLocaleDateString()}</span>
                        </div>
                        <p className="font-bold text-[13px] text-gray-800 mb-3 leading-snug">{survey.question}</p>
                          ...
                          {survey.options.map((option: any) => {
                            ...
                            return (
                              <button 
                                ...
                                disabled={hasVoted}
                                className={`w-full relative overflow-hidden p-2 border transition-all text-left ${
                                  isMyChoice ? "border-accent bg-accent/5" : "border-gray-200 hover:border-accent"
                                }`}
                              >
                                {hasVoted && (
                                  <div 
                                ...
                                <div className="relative z-10 flex justify-between items-center">
                                  <span className={`font-semibold text-xs ${isMyChoice ? "text-accent" : ""}`}>{option.text}</span>
                                  {hasVoted ? (
                                    <span className="text-[9px] font-black text-accent">%{percentage}</span>
                                  ) : (
                        ...
                        <div className="flex justify-between items-center text-[10px] text-gray-500 font-medium mt-4 pt-3 border-t border-gray-100">
                          <span>👤 {survey.interactions?.length || 0} Katılımcı</span>
                          <span>Oluşturan: {survey.club?.name}</span>
                        </div>
                      </div>
                    )) : (
                      <div className="text-center py-8 text-xs text-gray-400">Aktif anket bulunamadı.</div>
                    )}
                  </div>
                </div>
              </div>
