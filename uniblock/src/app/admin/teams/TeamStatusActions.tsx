...
import { handleTeamStatus } from "@/app/actions/admin";
...
export default function TeamStatusActions({ teamId, status }: TeamStatusActionsProps) {
  const [isLoading, setIsLoading] = useState(false);

  const onUpdate = async (newStatus: "ACTIVE" | "REJECTED" | "BANNED") => {
    setIsLoading(true);
    const result = await handleTeamStatus(teamId, newStatus);
    if (result.success) {
      toast.success("Takım durumu güncellendi.");
    } else {
